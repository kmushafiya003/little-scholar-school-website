...
const AboutModal = ({ modalData, setModalData }) => {
    ...
    return (
        ...
        <div className='fixed inset-0 z-[99999] grid lg:place-items-center justify-center bg-black bg-opacity-70 overflow-auto'>
            ...
            <div ref={modalRef} onClick={(events) => events.stopPropagation()} className='lg:w-[700px] mmd:w-[600px] sm:w-[70%] w-[80%] mx-auto  h-max bg-white flex flex-col lg:my-12 mt-32 mb-16'>

                {/* ---------- upper part ---------- */}

                <div className='flex  justify-between items-center sm:gap-x-8 gap-x-4 bg-dark-blue text-white px-6 py-4'>


                    {/* ----------- name ----------- */}
                    <p className='lg:text-4xl mmd:text-[30px] text-2xl uppercase font-semibold'>{modalData.name}</p>
                ...
                <div className='py-10 px-6 flex flex-col gap-y-5'>
                    ...
                    {/* ----------- position ------------- */}
                    <h1 className='lg:text-4xl smd:text-[30px] text-[24px] uppercase font-semibold text-black'>{modalData.position}</h1>
